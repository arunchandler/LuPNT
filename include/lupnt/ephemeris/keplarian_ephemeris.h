...
namespace lupnt {
  ...
  public:
     ...
     */
    EphemFitResult FitEphemeris(VecX t_fit, MatX fit_arc, Frame frame, EphemFitOption fit_option,
                                bool optimize = true);
     ...
     */
    Vec6d EvalFitError(EphemFitResult fit_result, bool is_plot, std::string fig_name,
                       bool plot_init);
  ...
  public:
     ...
     */
    std::map<std::string, Real> ComputeInternalParams(Real t, VecX ephemeris,
                                                      bool compute_vel = false);
     ...
     */
    std::map<std::string, Real> ComputeInitHarmCoeff(VecX fit_t, MatX fit_arc_bf, Real t_ref,
                                                     Vec6 coe_ref, bool plot_result = false);
  };
}  // namespace lupnt
