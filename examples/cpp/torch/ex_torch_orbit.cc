 ...
 * @brief  Compute the state transition matrix (STM) using Pytorch autograd (as potential
 * alternative to autodiff)
 * @version 0.1
 * @date 2024-12-22
 *
...
torch::Tensor crtbp_torch(double t, const torch::Tensor& x) {
  ...
  auto dx2 = x5;
  auto dx3 = x6;
  auto dx4 = 2 * x5 + x1 - (1 - mu) * (x1 + mu) / torch::pow(r1, 3)
             - mu * (x1 - 1 + mu) / torch::pow(r2, 3);
...
STMResultT propagate_with_stm_torch(torch::Tensor& x_init, double t0, double dt, int num_steps) {
