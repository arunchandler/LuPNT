...
int main() {
    ...
    EphemFitResult fit_result = ephemeris->FitEphemeris(fit_t, fit_x, inertial_frame, fit_option, true);
    ...
    filename += "/sim_" + std::to_string(int(sim_min)) + "_M0_" + std::to_string(int(M_init.val() * DEG));
    ...
    rms_error = ephemeris->EvalFitError(fit_result, is_plot, "true_estimated_position_init.png", true);

    std::vector<std::string> rms_labels = {"RMS Radial: ", "RMS Tangential: ", "RMS Normal: ", "RMS V_R: ", "RMS V_T: ", "RMS V_N: "};
    std::cout << " " << std::endl;
