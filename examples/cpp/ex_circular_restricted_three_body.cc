...
int main() {
    ...
    //Testing to see if Relativity stuff doesn't give me an error during setup
    RelativityClockDynamics relativity(integ);
    relativity.Propagate(VecX::Zero().cast<Real>(), static_cast<Real>(0), static_cast<Real>(1));
    ...
    Vec3d earth_pos(-mu.val(), 0.0, 0.0);      // Earth position in CR3BP
    Vec3d moon_pos((1.0 - mu).val(), 0.0, 0.0);  // Moon position in CR3BP
    ...
    scatter3(std::vector<double>{earth_pos(0)}, std::vector<double>{earth_pos(1)}, std::vector<double>{earth_pos(2)});
    scatter3(std::vector<double>{moon_pos(0)}, std::vector<double>{moon_pos(1)}, std::vector<double>{moon_pos(2)});
