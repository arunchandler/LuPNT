...
namespace lupnt {
  ...
  template <typename T> bool ConfigReader::InsertMap(std::map<std::string, Ptr<T>> &container,
                                                     std::string key, Ptr<T> value) {
    auto [iter, inserted] = container.insert({key, value});
    if (!inserted) {
      std::cerr << "[InsertMap] Key '" << key << "' already exists.\n";
  ...
  template <typename T>
  Ptr<T> ConfigReader::FindMap(std::map<std::string, Ptr<T>> &container, std::string key) {
    auto iter = container.find(key);
    if (iter == container.end()) {
      std::cerr << "[FindMap] Key '" << key << "' not found.\n";
  ...
  NaifId ConfigReader::FindFrameCenterMap(const std::map<Frame, NaifId> &container, Frame key) {
    auto iter = container.find(key);
    if (iter == container.end()) {
      std::cerr << "[FindMap] Key '" << key << "' not found.\n";
  ...
  template <typename T> T ConfigReader::LoadRequiredField(const YAML::Node &node,
                                                          const std::string &field_name /*= ""*/) {
    if (!node) {
      if (!field_name.empty()) {
        std::cerr << "[LoadRequiredField] Required field \"" << field_name << "\" not found.\n";
  ...
  template <> Real ConfigReader::LoadRequiredField(const YAML::Node &node,
                                                   const std::string &field_name /*= ""*/) {
  ...
  template <typename T>
  Ptr<NBodyDynamics<T>> ConfigReader::CreateNBodyDynamics(const YAML::Node child_node,
                                                          std::string name, bool print_val) {
    // Use LoadRequiredField for 'integrator'
    std::string integrator
        = LoadRequiredField<std::string>(child_node["integrator"], "dynamics.{}.integrator");
    ...
    if (child_node["bodies"]) {
      ...
      for (std::size_t i = 0; i < bodiesNode.size(); i++) {
        ...
        bool sphOrderMatch
            = (child_node["sph_order"] && (child_node["sph_order"].size() == bodiesNode.size()));
        bool sphDegreeMatch
            = (child_node["sph_degree"] && (child_node["sph_degree"].size() == bodiesNode.size()));

        if (sphOrderMatch) {
          n = child_node["sph_order"][i].as<int>();
        } else {
          std::cerr << "Spherical order does not match with number of bodies: "
                    << prior + ".sph_order\n";
        }
        if (sphDegreeMatch) {
          m = child_node["sph_degree"][i].as<int>();
        } else {
          std::cerr << "Spherical degree does not match with number of bodies: "
                    << prior + ".sph_degree\n";
        }
        // Create BodyT<double>
    ...
    if (child_node["integ_options"]) {
      ...
      iparams.abstol = LoadRequiredField<double>(child_node["integ_options"]["abstol"],
                                                 "dynamics.{}.integ_options.abstol");
      iparams.reltol = LoadRequiredField<double>(child_node["integ_options"]["reltol"],
    ...
    for (YAML::const_iterator it = dynamicsNode.begin(); it != dynamicsNode.end(); ++it) {
      ...
      if (dyn_type == "NBodyDynamics") {
        ...
        std::string model_name
            = LoadRequiredField<std::string>(child_node["model"], name + ".model");
  ...
  void ConfigReader::LoadChannelConfig(YAML::Node channelNode, bool print_val) {
    ...
    for (YAML::const_iterator it = channelNode.begin(); it != channelNode.end(); ++it) {
      ...
      std::string channel_type
          = LoadRequiredField<std::string>(child_node["type"], "channels." + name + ".type");
  ...
  void ConfigReader::LoadGnssConfig(YAML::Node gnssNode, bool print_val) {
    ...
    for (YAML::const_iterator it = gnssNode.begin(); it != gnssNode.end(); ++it) {
      ...
      std::string init_type = LoadRequiredField<std::string>(child_node["initialization"],
                                                             field_prior + ".initialization");

      if (init_type == "TLE") {
        if (is_first_const) {
          // Check if we have the required dynamics
          std::string dyn_key
              = LoadRequiredField<std::string>(child_node["dynamics"], "gnss.dynamics");
  ...
  void ConfigReader::CreateOrbitState(YAML::Node state_node, Ptr<Spacecraft> sat,
    ...
    if (state_node["class"].as<std::string>() == "CartesianOrbitState") {
      ...
      std::string init_type
          = LoadRequiredField<std::string>(state_node["init"]["type"], "orbit.init.type");
      ...
      if (init_type == "ClassicalOE") {
        ...
        Vec6 coe_vec = {
            ...
            LoadRequiredField<Real>(state_node["init"]["val"]["Omega"],
                                    prior_key + "init.val.Omega")
                * RAD,
            LoadRequiredField<Real>(state_node["init"]["val"]["w"], prior_key + "init.val.w") * RAD,
            LoadRequiredField<Real>(state_node["init"]["val"]["M"], prior_key + "init.val.M")
                * RAD};
        ...
        std::cerr << "Invalid initialization type for Orbit State: must be ClassicalOE or "
                     "CartesianOrbitState.\n";
      }
    } else {
  ...
  void ConfigReader::CreateClockState(YAML::Node state_node, Ptr<Spacecraft> sat,
                                      std::string prior_field) {
    // We expect "val.bias" and "val.drift" at minimum
    Real bias = Real(LoadRequiredField<double>(state_node["init"]["val"]["bias"],
                                               prior_field + ".init.val.bias"));
    Real drift = Real(LoadRequiredField<double>(state_node["init"]["val"]["drift"],
                                                prior_field + ".init.val.drift"));
    ...
    if (state_node["dynamics"]) {
      auto clock_dyn
          = FindMap<ClockDynamics>(clock_dynamics_map_, state_node["dynamics"].as<std::string>());
  ...
  void ConfigReader::CreateSatCommDevice(YAML::Node device_node, std::string device_name,
                                         Ptr<Spacecraft> sat) {
    std::string prior_field = "device." + device_name;
    std::string device_class
        = LoadRequiredField<std::string>(device_node["class"], prior_field + ".class");
    std::string device_channel
        = LoadRequiredField<std::string>(device_node["channel"], prior_field + ".channel");

    if (device_class == "GnssReceiver") {
      Ptr<GnssReceiver> device;
      std::string rcvr_name
          = LoadRequiredField<std::string>(device_node["name"], prior_field + ".name");
      ...
      std::cerr << "Invalid Device type for '" << device_name
                << "': only GnssReceiver supported.\n";
      return;
    }
  }
  ...
  void ConfigReader::CreateSatState(YAML::Node state_node, std::string state_name,
                                    Ptr<Spacecraft> sat, bool &orbit_defined, bool &clock_defined) {
    // "class" is mandatory
    std::string prior_field = "satellites." + sat->GetName() + ".states." + state_name;

    std::string class_name
        = LoadRequiredField<std::string>(state_node["class"], prior_field + ".class");

    if (class_name == "CartesianOrbitState") {
      if (orbit_defined) {
        std::cerr << "Orbit State already defined for Agent '" << sat->GetName() << "'\n";
        return;
      }
      ...
      if (clock_defined) {
        std::cerr << "Clock State already defined for Agent '" << sat->GetName() << "'\n";
        return;
      }
    ...
    for (YAML::const_iterator it = satellite_node.begin(); it != satellite_node.end(); ++it) {
      ...
      if (child_node["states"]) {
        for (YAML::const_iterator sit = child_node["states"].begin();
             sit != child_node["states"].end(); ++sit) {
          std::string state_name = sit->first.as<std::string>();
          YAML::Node state_node = sit->second;
          CreateSatState(state_node, state_name, satellites_[idx], orbit_defined, clock_defined);
        }
      }
      ...
      if (child_node["device"]) {
        for (YAML::const_iterator devIt = child_node["device"].begin();
             devIt != child_node["device"].end(); ++devIt) {
  ...
  Ptr<JointState> ConfigReader::CreateJointState(YAML::Node dynamics_list,
                                                 std::string field_prior) {
    ...
    for (YAML::const_iterator it = dynamics_list.begin(); it != dynamics_list.end(); ++it) {
      ...
      std::string state_str
          = LoadRequiredField<std::string>(child_node["state"], field_prior + ".state");
      Ptr<IState> state = FindMap<IState>(state_map_, state_str);

      // dynamics
      std::string dyn_str
          = LoadRequiredField<std::string>(child_node["dynamics"], field_prior + ".dynamics");
  ...
  void ConfigReader::LoadFiltersConfig(YAML::Node filtersNode, bool print_val) {
    ...
    for (YAML::const_iterator it = filtersNode.begin(); it != filtersNode.end(); ++it) {
      ...
      std::string filter_type
          = LoadRequiredField<std::string>(child_node["type"], "filters." + name + ".type");
      ...
      if (child_node["dynamics_list"]) {
        auto joint_state
            = CreateJointState(child_node["dynamics_list"], "filter." + name + ".dynamics_list");
