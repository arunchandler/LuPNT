...
namespace lupnt {
        ...
        if (optimize){
            ...
            auto objfunc = [this, t_ref, t_fit, fit_arc_pos_bf, lent](const VecX x, MatXd* J) {
                ...
                for (int i = 0; i < lent; i++) {
                    ...
                    if (J != nullptr) {
                        ...
                        VecXd J_tmp_vec = Jtmp.block(0, 1, 3, ephem_size_-1);   // (1x3) x (3 x ephem) = (1xephem)
            ...
            * ******************************************/
            ...
            for(int iter = 0; iter < fit_option.max_iter; iter++) {
                ...
                // std::cout << "cost: " << cost << " cost_new: " << cost_new << " rho: " << rho << " rho_tmp: " << rho_tmp << std::endl;
                ...
                if (rho > fit_option.eps_lm) {
                    ...
                    if (debug){
                        std::cout << "  (Improved)     Iter: " << iter << " Cost: " << cost << " Lambda: " << lambda << "  rho:" << rho << std::endl;
                    }
                }
                else {
                    lambda = MinD(lambda * fit_option.lm_up, fit_option.lm_max);
                    if (debug){
                        std::cout << "  (Not-improved) Iter: " << iter << " Cost: " << cost << " Lambda: " << lambda << "  rho:" << rho << std::endl;
                    }
                }
            }  // end of iteration 
        } // end of optimization
    ...
    Vec3 IEphemeris::EphemerisToFixedFramePos(Real t, VecX ephemeris, MatXd* J) {
        auto func = [=, this](const VecX ephem) {
            return EphemerisToFixedFramePos(t, ephem);
        };
   ...
   Vec6d IEphemeris::EvalFitError(EphemFitResult fit_result, bool is_plot, std::string figname,
        ...
        MatXd rv_true(lent,6), rv_est(lent,6), rv_init(lent,6), rtn_est(lent,6), rtn_init(lent,6), v_rot_true(lent,3);
        ...
        std::vector<std::string> ylabels = {"Radial Position [m]", "Tangential Position [m]", "Normal Position [m]", 
        ...
        switch (type) {
            ...
            case KepEphemType::EPH22:
                ...
                ephemeris_names_ = {"tref",                 // Reference Epoch
                    ...
                    "delta_n", "dot_i", "dot_Omega",            // Secular rates
                    "Cuc",  "Cus", "Crc", "Crs", "Cic", "Cis",   // Harmonic coefficients
                    "dot_u", "dot_r", "Clc", "Cls", "Crc2", "Crs2"  // additional Harmonic coefficients
                };
                break;
            default:
                break;
    ...
    Vec6 KeplarianEphemeris::EphemerisToFixedFramePosVel(Real t, VecX ephemeris) {
        ...
        // velocity terms 
        // Reference: https://www.gps.gov/technical/icwg/meetings/2019/09/GPS-SV-velocity-and-acceleration.pdf
    ...
    std::map<std::string, Real> KeplarianEphemeris::ComputeInternalParams(Real t, VecX ephemeris, bool compute_vel) {
        ...
        if (compute_vel) {
            ...
            Real rdot = e * a * Edot * sin(E_k) + dot_r + 2 * nudot * (Crs * cos(Phi) - Crc * sin(Phi) + 2 * Crc2 * cos(2 * Phi) - 2 * Crs2 * sin(2 * Phi));
    ...
    std::map<std::string, Real> KeplarianEphemeris::ComputeInitHarmCoeff(VecX fit_t, MatX fit_arc_bf, Real t_ref, Vec6 coe_ref, 
                                                                         bool plot_result) {
        ...
        for (int i = 0; i < lent; i++){
            ...
            Phi2dot_mat.row(i) << cos(Phi_i).val(), sin(Phi_i).val(), cos(2 * Phi_i).val(), sin(2 * Phi_i).val(), (fit_t(i) - t_ref).val(), 
        ...
        if (plot_result) {
            ...
            // Convert to double
            std::vector<double> delta_rp, delta_up, delta_ip, delta_omegap, delta_rpest, delta_upest, delta_ipest, delta_omegaest, cos_Phi, sin_Phi, ddelta_omega;
            ...
            if (save_path_set_) {
                ...
                std::ofstream file(save_path_ / csv_name);
                file << "Time,delta_r,delta_u,delta_i,delta_omega,delta_r_est,delta_u_est,delta_i_est,delta_omega_est\n";
                for (int i = 0; i < lent; i++) {
                    file << plot_t[i] << ", " << delta_rp[i] << ", " << delta_up[i] << ", " << delta_ip[i] << ", " << delta_omegap[i] << ", " << delta_rpest[i] << ", " << delta_upest[i] << ", " << delta_ipest[i] << ", " << delta_omegaest[i] << "\n";
                }
                file.close();
            }
