...
namespace lupnt {
  ...
  MatX UKF::ComputeSigmaPoints(const VecX& state, const MatXd& cov) {
  ...
  void UKF::UnscentedTransform(const MatX& sigma_points, VecX& state_mean, MatXd& cov_out,
                               bool add_noise, const MatXd& Q) {
    // Compute the mean of the sigma points
    state_mean = sigma_points * w_m_;
    ...
    if (add_noise) {
      cov_out += Q;  // Add process noise if required
    }
  }

  void UKF::Predict(Real t_end) {
    ...
    for (int i = 0; i < n_sigma_; i++) {
      // f_dyn_ signature: f_dyn_(x, t_curr, t_end, MatXd* F)
      // we pass nullptr for the Jacobian
      sigma_points_pred.col(i) = f_dyn_(sigma_points.col(i), t_, t_end, nullptr);
    }

    // 3) Compute predicted mean/cov + add process noise
    MatXd Q_proc = f_proc_(x_, t_, t_end);  // Q for process noise

    VecX state_pred(n_x_);
    MatXd cov_pred(n_x_, n_x_);
    UnscentedTransform(sigma_points_pred, state_pred, cov_pred, true, Q_proc);
