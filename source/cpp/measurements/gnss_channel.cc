...
namespace lupnt {
   ...
   */
  std::vector<GnssTransmission> GnssChannel::Receive(Ptr<GnssReceiver> &rx, Real t) {
    std::vector<GnssTransmission> received_transs;
    for (auto &tx : tx_devices) {
      auto tx_tmp = std::dynamic_pointer_cast<Transmitter>(tx);

      Real t_rx = t;
      Real t_tx = SolveLightTimeDelayTx(tx.get(), rx.get(), t_rx);

      CartesianOrbitState rv_rx_gcrf = rx->GetAgent()->GetCartesianGCRFStateAtEpoch(t_rx);
      CartesianOrbitState rv_tx_gcrf = tx->GetAgent()->GetCartesianGCRFStateAtEpoch(t_tx);
      ...
      if (occult["earth"] || occult["moon"]) continue;
