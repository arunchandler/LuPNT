...
class AStarPlanner(AStar):
    ...
    def plan(self, start, goal):
        ...
        path = np.array(path, dtype=dt)[
            :, [1, 0]
        ]  # flip x and y to get back to spatial coordinates
        self.path = path
        return path
...
class AStarGradPlanner(AStar):
    ...
    def grid_plan(self, start_idx, goal_idx):
        ...
        )  # path is in image coordinates
        dt = np.dtype("int32", "int32")
        path_idxs = np.array(path_idxs, dtype=dt)[
            :, [1, 0]
        ]  # flip x and y to get back to grid coordinates
        return path_idxs
